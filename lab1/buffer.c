...
Buffer *copy_buffer(Buffer *buffer) {
    Buffer *copied_buffer = (Buffer*)malloc(sizeof(Buffer));
    copied_buffer -> data = (char*)malloc(buffer -> size);
    copied_buffer -> size = buffer -> size;
    for(int i = 0; i < buffer -> size; i++){
        copied_buffer -> data[i] = buffer -> data[i];
    }
